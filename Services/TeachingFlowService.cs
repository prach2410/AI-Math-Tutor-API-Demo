...
public class TeachingFlowService(AppDbContext db, IChatProvider chat)
{
    ...
    private const string SolvePrompt = """
        ...
        - ถ้าโจทย์ใช้สูตร/หลักการเฉพาะ (พีทาโกรัส a²+b²=c², พื้นที่, ปริมาตร, ดอกเบี้ย ฯลฯ):
          ต้องมีขั้น "ระบุสูตร" ชัดเจน (ป้าย = ชื่อสูตร · บรรทัด = สูตรทั่วไป) ก่อนแทนค่าเสมอ
          ❌ ห้ามข้ามจากโจทย์ไปแทนตัวเลข/คำนวณเลยโดยไม่โชว์สูตรที่ใช้ (เด็กจะลอกคำตอบโดยไม่เห็นหลักการ)
        ...
        ตัวอย่าง 4 — พีทาโกรัส หา x (สามเหลี่ยมมุมฉาก ด้านประกอบมุมฉาก 6 และ 8, ด้านตรงข้ามมุมฉาก x):
        solutionSteps: [
          "ระบุสูตร (พีทาโกรัส)\na² + b² = c²    (c = ด้านตรงข้ามมุมฉาก)",
          "แทนค่า\n6² + 8² = x²",
          "คำนวณทีละบรรทัด\n36 + 64 = x²\n    100 = x²",
          "ถอดราก\nx = √100\nx = 10"
        ]
        keyStepIndices: [1]   ← ขั้น 1 "ระบุสูตร" คือหลักการที่ต้องรู้ · ขั้น 2-4 แทนค่า/คำนวณตรงไปตรงมา
